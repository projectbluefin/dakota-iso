...
|---|---|---|---|
| Bluefin | `build-iso-bluefin.yml` | `bluefin-live-latest.iso` | manual dispatch only |
| Bluefin LTS HWE | `build-iso-bluefin.yml` | `bluefin-lts-hwe-live-latest.iso` | manual dispatch only |
...
| Bluefin Build & Publish | `build-iso-bluefin.yml` | `workflow_dispatch` only — schedule disabled 2026-09-18 |
...
**Triggers:** `workflow_dispatch` only — the daily `schedule` was commented out on
2026-09-18 when this repo narrowed to Dakota. Nothing invokes this automatically.
