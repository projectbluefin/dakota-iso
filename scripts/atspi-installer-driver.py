...
class AtspiSession:
    def __init__(self, atspi):
        self.atspi = atspi
        self.desktop = atspi.Registry.getDesktop(0)
    ...
    def is_checked(self, node) -> bool:
        try:
            return node.getState().contains(self.atspi.STATE_CHECKED)
        except Exception:
            return False

    def is_sensitive(self, node) -> bool:
        try:
            return node.getState().contains(self.atspi.STATE_SENSITIVE)
        except Exception:
            return True
    ...
    def click(self, node, purpose: str) -> None:
        try:
            ...
            available = [
                (index, normalized(action.getName(index)))
        ...
        if len(available) == 1 and action.doAction(available[0][0]):
            return
    ...
    def _is_visible(self, node) -> bool:
        try:
            state = node.getState()
            return state.contains(self.atspi.STATE_VISIBLE) and state.contains(
                self.atspi.STATE_SHOWING
            )
        except Exception:
            return True
...
def parse_args() -> argparse.Namespace:
    ...
    parser.add_argument("--disk", required=True, help="whole target device, e.g. /dev/vda")
    parser.add_argument(
        "--timeout",
        type=float,
        default=3600,
    ...
    if not re.fullmatch(r"/dev/[A-Za-z0-9._-]+", args.disk):
...
def main() -> int:
    ...
    try:
        ...
        print(
            "AT-SPI driver requires pyatspi in the live GNOME session.",
            file=sys.stderr,
        )
        return 2
